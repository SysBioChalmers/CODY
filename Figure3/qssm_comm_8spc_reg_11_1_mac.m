...
for k=1:n_rct-1
    BIOM_k=[];
    BIOM_k=y((k-1).*ylg+1:(k-1).*ylg+ns);   %% all bacteria concentration(1:ns) in current tank
%     rhyd(:,k)=kmax_hmo.*HMO(k).*BIOM_k./(KM_hmo.*BIOM_k+HMO(k)).*spc_hyd;     %%??????????????????????????????????????????????????????????????????????????????????
%     rhyd(:,k)=kmax_hmo.*HMO(k).*BIOM_k./(KM_hmo.*BIOM_k+113*HMO(k)).*spc_hyd;     %%??????????????????????????????????????????????????????????????????????????????????????
    rhyd(:,k)=kmax_hmo.*HMO(k).*BIOM_k./(KM_hmo.*BIOM_k+HMO(k)).*spc_hyd;     %%?????????????????????????????????????????????????????????????????????????????????????
...
%         Flow_L{k}=JLT{k}*V_colon(k)/rho;  %%??????????????????????????????????????????????????????????????????????????????????????????????
    ...
    for k=5:8
        for i=ns+1:lx
%             Mass_mucosa_temp=Mass_mucosa_temp+mol_mass(i-ns).*(y(i+(k-1)*ylg)-y(i-ns+icr_blood));    %% ????????????????????????????????????????????????????????????????)
 ...
 tc=24;   
 T=5/60;   
 [V5,F5]=calc_VF(V5,F5,t,T,V_colon);
...
for k=1:rct-1   
  for s=1:ns
    ...
    for k=1:rct-1  %%????????????dy??????1??????lx?????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
        for s=1:ns   
            dxdt_fnl(1:lx)=dxdt_fnl(1:lx)+dxdt{k}{s}(1:lx);  %%1??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????dy????????????????????????????????????????????????????????????????????????????????????????????????????????????
        end
