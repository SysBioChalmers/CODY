function dy=qssm_comm_10_rct_HMO(t,y,deltat,para,ns,FS,FV,V_colon,sub_index,hmo_in_y,enz_inx_y)
% function dy=qssm_comm_10_rct_HMO(t,y,para,ns,FS,FV,V_colon,sub_index,hmo_in_y,enz_inx_y)
...
%% met_udf_co_new?????????????????????????????????????????biofilm???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????met_udf_co??????????????????????
for i=1:length(y)
...
k=10;  %%?????????????????????????????????????????????????????????????????????????????????????????????????????????
    ...
    for k=1:4
        for i=ns+1:lx
            Mass_temp=Mass_temp+mol_mass(i-ns).*(y(i+(k-1)*ylg));    %% ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????hmo?????????????????????????????????????????????hmo?????????????????????????????????????????????????????1000????????????????????????????????????????????????????????????????????????????????????????????gram????????????            
 ...
 tc=24/3;   
 T=1/20;  
[V5,F5]=calc_VF_4m(V5,F5,t,T,V_colon);
...
%     sub=substrate{k}{s};   %% ???????????????????????
    sub=sub_mdf{k}{s};   %% ????????????????????????????????
    KS=KM{s};
    BIOM=y(s+(k-1).*ylg);  %%% ??????????????????????????????????????????????????k???????????????????????????????????s???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
    SxZ=SxZ_tot{s};
    klz=length(kmax{s});
    S=[];
   
    for i=1:length(sub)   %%% number of substrate
      S(:,i)=sub(i).*ones(klz,1);  %%% Subsrate, 
%     S(:,ss)=sub(ss).*ones(length(kmax{s}),1);
    end
...
%      icr=k.*ylg;
     ...
     dy(1+icr:ns+icr)=dy(1+icr:ns+icr)-FL(k+1)/V_colon(k)*y(1+icr:ns+icr)+f_back(k+1)/V_colon(k).*y((1+icr_next):(ns+icr_next))+detachment(:,k);        %% biomass of each species in the fluid  
%      dy(ind_hex)=dy(ind_hex)+FL0/V_colon(k).*GLU_in-FL(k+1)/V_colon(k)*y(ind_hex)+f_back(k+1)/V_colon(k).*y(ind_hex+icr)-km(1)*(y(ind_hex)-y(ind_hex+icr_m));   %%????????????????????????????????FS(1)
     dy(hmo_ix+icr)=dy(hmo_ix+icr)+FL0/V_colon(k).*HMO_in-FL(k+1)/V_colon(k)*y(hmo_ix+icr)+f_back(k+1)/V_colon(k).*y(hmo_ix+icr_next)-km(1)*(y(hmo_ix+icr)-y(hmo_ix+icr_m))-r_hyd_hmo{k};   %%????????????????????????????hmo??????????????????????????????hmo??????????????????g/L??????????????????????????????????????????????????????
     dy(ind_hex+icr)=dy(ind_hex+icr)-FL(k+1)/V_colon(k)*y(ind_hex+icr)+f_back(k+1)/V_colon(k).*y(ind_hex+icr_next)-km(2)*(y(ind_hex+icr)-y(ind_hex+icr_m))+r_hyd_hex{k};   %%????????????????????????????hexose????????????????????????????????????????????????????????????????????????????????????????????????????????????????????%%
...
k=4;    %% VL4, different with VL1-VL3, no f_back(k+1) term
 ...
 icr_lm=(k-1-4).*ylg;
 icr_bld=9*ylg+1+lx;
%      dy(1+icr:ns+icr)=dy(1+icr:ns+icr)-kd(k-4).*y(1+icr:ns+icr);   %% X bacteria in mucosa, only growth and detachment
     detachment(:,k)=kd(k-4).*y(1+icr:ns+icr)./(C_Kd(k-4)+y(1+icr:ns+icr)).*y(1+icr:ns+icr);
%      dy(1+icr:ns+icr)=dy(1+icr:ns+icr)-detachment(:,k-4).*V_colon(k-4)./V_colon(k);   %% X bacteria in mucosa, only growth and detachment
 ...
 k=9;    %%
     ...
     icr_next=(k).*ylg;
               ...
               dx_dt_V5_temp=dy(1+icr:ns+icr);dSP_dt_V5_temp=dy(hmo_ix+icr:lx+icr); %% ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????d(x1V5-x5V5)/dt,d(HexoseV5-H2V5)/dt?????????????????????????????????????||??????????????hmo????????????????????
